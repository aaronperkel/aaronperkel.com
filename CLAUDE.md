...
- `components/ProjectsList.tsx` — the one interactive piece: projects render as a man-page style definition list of native `<details>` entries (name, stack, and tagline always visible; description and screenshot on expand). Opening `/?project=<Name>` expands that entry and scrolls to it (resume "Projects" links and old indexed URLs rely on this).
- `components/ManPage.tsx` — the man-page chrome every page opens and closes with: the running head (`PERKEL(1) · User Commands · PERKEL(1)`) and `ManSection`, the flush-left uppercase section head with an indented body.
