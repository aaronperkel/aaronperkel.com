...
- Design tokens (page/panel/primary/muted/accent colors) are Tailwind `@theme` variables in `app/globals.css`, carried over from the PHP site's stylesheet. Styling is Tailwind utilities in JSX; globals.css has only base element styles.
